...
// Define interfaces for raw Strapi data structure (no 'attributes' wrapper)
interface Order {
  id: number;
  order_number: string;
  total_amount: number;
  ...
  order_items: Array<{
    ...
    order_item_parts: Array<{
      ...
      color: {
        id: number;
        name: string;
        hex_codes: Array<{ hex_code: string }>;
      };
    }>;
    promotions: Array<{
      id: number;
...
<head>
  <style>
    ...
    .container {
      ...
      border: 3px solid;
      border-image: linear-gradient(45deg, #fe5100, white, #fe5100) 1;
    }
    ...
    .button:hover {
      ...
      background: linear-gradient(45deg, #fe5100, white, #fe5100);
      color: #333;
    }
...
export default factories.createCoreService('api::webhook-event.webhook-event', ({ strapi }) => ({
  async processWebhookEvent(event: any) {
    ...
    if (event_type === 'payment_intent.succeeded') {
      ...
      strapi.log.info(`Handling payment_intent.succeeded for payment_intent_id: ${paymentIntentId}`);
      ...
      strapi.log.info(`Found ${orders.length} orders for payment_intent_id: ${paymentIntentId}`);
      if (orders.length === 0) {
      ...
      strapi.log.info(`Processing order ${order.id}`);
      ...
      try {
        ...
        if (paymentIntent.latest_charge) {
          ...
          if (charge.payment_method_details?.card?.last4) {
            ...
            strapi.log.info(`Set payment_last_four to ${updateData.payment_last_four} for order ${order.id}`);
          }
        }
        ...
        strapi.log.info(`Updated order ${order.id} with payment details`);
        ...
        strapi.log.error(`Error updating order ${order.id}: ${(error as Error).message}`);
        throw error;
      }
    } else if (event_type === 'charge.succeeded') {
      const paymentIntentId = event_data.payment_intent;
      strapi.log.info(`Handling charge.succeeded for payment_intent_id: ${paymentIntentId}`);
      ...
      if (orders.length > 0) {
        ...
        if (order.payment_status !== 'completed') {
          ...
          strapi.log.info(`Order ${order.id} already completed; skipping update`);
        }
      }
    } else {
  ...
  async sendConfirmationEmail(order: Order) {
    ...
    try {
      ...
      strapi.log.info(`Successfully sent confirmation email to ${customerEmail}`);
    } catch (emailError) {
      strapi.log.error(`Failed to send confirmation email to ${customerEmail}: ${(emailError as Error).message}`);
    }
  },
  ...
  transformOrderData(order: Order) {
    return {
      ...
      total_amount: (order.total_amount / 100).toFixed(2), // Convert cents to dollars
